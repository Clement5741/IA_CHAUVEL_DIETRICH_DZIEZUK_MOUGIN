...
public class AStar extends TreeSearch {
    ...
    public AStar(SearchProblem prob, State initial_state) {
        ...
        // Initialisation de la frontière comme une PriorityQueue triée par (g + h)
        this.frontier = new PriorityQueue<>(Comparator.comparingDouble(node -> node.getCost() + node.getHeuristic()));
    }
    ...
    public boolean solve() {
        ...
        while (!this.frontier.isEmpty()) {
            // 5. Retirer un noeud de la frontière avec le coût estimé le plus bas (g + h)
            ...
            for (Action action : problem.getActions(currentState)) {
                ...
                if (!frontier.contains(childNode) && !explored.contains(childState)) {
                    ...
                    // Si le noeud est déjà dans la frontière mais avec un coût estimé inférieur, on le met à jour
                    for (SearchNode node : frontier) {
                        if (node.equals(childNode) && (node.getCost() + node.getHeuristic() > childNode.getCost() + childNode.getHeuristic())) {
