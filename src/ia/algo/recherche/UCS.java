...
public class UCS extends TreeSearch {
    ...
    public UCS(SearchProblem prob, State initial_state) {
        ...
        // Initialisation de la frontière comme une PriorityQueue triée par coût
        this.frontier = new PriorityQueue<>(Comparator.comparingDouble(SearchNode::getCost));
    }
    ...
    public boolean solve() {
        ...
        while (!this.frontier.isEmpty()) {
            // 5. Retirer un noeud de la frontière avec le coût le plus bas
            ...
            for (Action action : problem.getActions(currentState)) {
                ...
                if (!frontier.contains(childNode) && !explored.contains(childState)) {
                    ...
                    // Si le noeud est déjà dans la frontière mais avec un coût supérieur, on le met à jour
                    for (SearchNode node : frontier) {
                        if (node.equals(childNode) && node.getCost() > childNode.getCost()) {
